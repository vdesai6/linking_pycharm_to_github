...
filepath_imgs['ID'] = filepath_imgs.apply(lambda x: x.split('_')[-1][:-4])
...
#images = pd.concat([filepath_imgs, dm.DM], axis=1, ignore_index=True).sample(frac=1.0, random_state=1).reset_index(drop=True)
print(images)
